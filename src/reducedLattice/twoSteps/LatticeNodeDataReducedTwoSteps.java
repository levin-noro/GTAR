...
public class LatticeNodeDataReducedTwoSteps implements ILatticeNodeData {
	...
	// usage: for root node;
	public LatticeNodeDataReducedTwoSteps(Set<String> set) {
	 ...
	 */
	public LatticeNodeDataReducedTwoSteps(PatternNode sourceAbstractPatternNode,
	 ...
	 * @param direction
	 */
	public LatticeNodeDataReducedTwoSteps(ListenableDirectedGraph<PatternNode, DefaultLabeledEdge> newAbsPattern,
	...
	private boolean twoPatternNodesAreSame(PatternNode unexpandedPatternNode,
			...
			LatticeNodeDataReducedTwoSteps LatticeNodeData) {
	...
	public void initRHSQueueFromChildrenMaxRules(ILattice lattice, LatticeNode<ILatticeNodeData> thisNode,
		...
		//
		// HashSet<LatticeNode<ILatticeNodeData>> suitableRHSs = new
		// HashSet<LatticeNode<ILatticeNodeData>>();
		// if (thisNode.getChildrenLinksSet() == null ||
		// !havingPotentialChildren(thisNode)) {
		// rhsQueue.addAll(lattice.getLatticeNodeIndex().get(0).getChildren());
		// } else {
		// for (LatticeNode<ILatticeNodeData> child :
		// thisNode.getChildrenLinksSet()) {
		// for (Rule rule : child.getData().getRulesOfThis()) {
		// removeSubPatternKeepSuperPattern(lattice, suitableRHSs, rule.rhs);
		// }
		// }
		//
	...
	private boolean havingPotentialChildren(LatticeNode<ILatticeNodeData> thisNode) {
		...
		for (LatticeNode<ILatticeNodeData> child : thisNode.getChildrenLinksSet()) {
			if (child.getData().canBeFrequent() && child.getData().getRulesOfThis().size() > 0) {
				return true;
			}
		}
		return false;
	...
	private void removeSubPatternKeepSuperPattern(ILattice lattice, HashSet<LatticeNode<ILatticeNodeData>> suitableRHSs,
		...
		while (itr.hasNext()) {
			...
			if ((DualSimulationHandler.checkIfSubPattern(lattice,
					lattice.getLabelAdjacencyIndexer().subPatternsOfAPattern, rhs, latticeNode))) {
				authorizedToBeThere = false;
				break;
			}
			if (DualSimulationHandler.checkIfSubPattern(lattice,
					lattice.getLabelAdjacencyIndexer().subPatternsOfAPattern, latticeNode, rhs)) {
				itr.remove();
			}
		}
