...
//
//public class LatticeReducedTwoSteps implements ILattice {
...
//	Queue<LatticeNode<ILatticeNodeData>> lhsQueue;
...
//	 * @throws Exception
//	 */
//	public LatticeReducedTwoSteps(String focusSetPath, int maxAllowedHops, int maxAllowedEdges,
...
//		ILatticeNodeData emptyPTRootData = new LatticeNodeDataReducedTwoSteps(focusLabelSet);
...
//		lhsQueue = new LinkedList<LatticeNode<ILatticeNodeData>>();
//
...
//		}
//		ILatticeNodeData firstLevelChildData = new LatticeNodeDataReducedTwoSteps(focusNode, dgGraphMatchNodes,
...
//	}
//
//	public LatticeNode<ILatticeNodeData> generateLatticeForTemporalGraph() throws Exception {
//
...
//
//		if (lhsProcessingNode.getChildren() != null) {
//			for (LatticeNode<ILatticeNodeData> newChild : lhsProcessingNode.getChildren()) {
//				innerQueueNodePostProcessing(lhsProcessingNode, newChild, expansionSide);
//			}
//		}
//	}
//
//	private void innerQueueNodePostProcessing(LatticeNode<ILatticeNodeData> lhsProcessingNode,
//			LatticeNode<ILatticeNodeData> newChild, ExpansionSide expansionSide) throws Exception {
//
...
//		if (newChild.getData().getNumberOfDistinctFocusesOverAllTimestamps() > 0
//				&& newChild.getData().getTotalSupportFrequency() >= supportThreshold) {
//
//			newChild.getData().setCanBeFrequent(true);
//
//			if (!lhsQueueSet.contains(newChild)) {
...
//			HashMap<PatternNode, HashSet<LatticeNode<ILatticeNodeData>>> newChildrenOfTheSrc, String unSeenFocusType,
...
//		}
//		if (unSeenFocusType != null) {
//			sameLabelNeighborNodes.retainAll(allNodesOfFocusType.get(unSeenFocusType));
//		}
//
...
//				if (destInDegree > dataGraph.getNodeById(newNodeId).getDegree(org.neo4j.graphdb.Direction.INCOMING)) {
//
...
//
//			for (Integer newNodeId : destPatternNode.newNodeIds) {
//				if (incomingFromSameType > labelAdjacencyIndexer.temporalNeighborhoodIndex.get(newNodeId)
...
//						destPatternNode, destPatternNode.newNodeIds,
...
//							latticeProcessingLevel, srcPatternNode, seenPatternNodeFromPreviousNodesForThisSrcIncoming,
...
//								otherNodeLabelRelType, snapshot, latticeProcessingLevel, newChildrenOfTheSrc, null,
//								expansionSide);
//					}
//					for (String otherNodeLabelRelType : labelAdjacencyIndexer.temporalNeighborhoodIndex
//							.get(srcDataGpNodeId).keySet()) {
...
//						latticeProcessingLevel, newChildrenOfTheSrc, unSeenFocusType, expansionSide);
//			}
//		}
//		for (String otherNodeLabelRelType : labelAdjacencyIndexer.temporalNeighborhoodIndex.get(srcDataGpNodeId)
//				.keySet()) {
...
//			HashSet<Integer> newNodeIdsMustBeRemoved = new HashSet<Integer>();
//			for (Integer newNodeId : destPatternNode.newNodeIds) {
//				if (destOutDegree > dataGraph.getNodeById(newNodeId).getDegree(org.neo4j.graphdb.Direction.OUTGOING)) {
//
//					newNodeIdsMustBeRemoved.add(newNodeId);
//				}
//			}
...
//		}
//
//		ILatticeNodeData LatticeNodeData = new LatticeNodeDataReducedTwoSteps(newPatternGraph,
...
//
//		// if (numberOfPatternsInLattice == 11) {
//		// System.out.println();
//		// }
//
//		if (DummyProperties.debugMode) {
...
//			}
//		}
//		if (remainingNodeIds.size() > 0 && remainingNodeIds.contains(srcDataGpNodeId)) {
//			// handling self-loop
//			HashSet<Integer> goBackToPrevNodeIds = new HashSet<Integer>();
