						...
						for (double c : confidenceThresholds) {
							...
							for (int t : deltaTs) {
								...
								for (int exp = 0; exp < numberOfSameExperiments; exp++) {
									GTARFinderStat gTARFinderStat = new GTARFinderStat("focusSet.txt", h,
											e, dataGraphPath, debugMode, s, c, t);
									gTARFinderStat.findGTARs();

									String settingStr = " focus was " + line + ", h:" + h + " , e:" + e + ", s:" + s
											+ ", c:" + c + ", deltaT:" + t;
									System.out.println("OPT gTARFinderStat Finder: exp " + exp + settingStr);

									DebugHelper.printingRules(bwHeuristic, gTARFinderStat.lattice, settingStr);
